...
def start_command(message):
    bot.send_message(message.chat.id, """Привет! Я твой помощник в управлении проектами. Для того чтобы увидеть что я могу --> /info :) 
""")
...
• /projects — посмотреть список всех проектов  
• /delete — удалить ненужный проект  
• /update_projects — обновить информацию о проекте

💡 А ещё ты можешь просто ввести название проекта, и я покажу всю информацию о нём.""")
...
def get_projects(message):
    bot.send_message(message.chat.id, 'вот', reply_markup=types.ReplyKeyboardRemove())
    user_id = message.from_user.id
    projects = manager.get_projects(user_id)
